...
class _StokMasukPageState extends State<StokMasukPage> {
  ...
  Future<void> _fetchVendor() async {
    try {
      final url = Uri.parse('http://192.168.116.127:5002/api/get_vendor?username=$_username');
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'username': _username}),
      ...
      if (response.statusCode == 200) {
        final List vendorData = json.decode(response.body);
        setState(() {
          _vendorList = vendorData.map((vendor) {
            return {
              'namavendor': vendor['namavendor'].toString(),
            };
          }).toList();
        });
      } else {
      ...
      print('Error pada _fetchVendor: $e');
    }
  }
  ...
  Future<void> _submitData() async {
    ...
    try {
      ...
      final uri = Uri.parse('http://192.168.116.127:5002/api/stok_masuk?username=$_username&kodebarang=$_selectedKodeBarang');
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(labelText: 'Vendor (Opsional)'),
              ),
              const SizedBox(height: 16),
              TextFormField(
                readOnly: true,
